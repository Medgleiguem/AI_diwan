...
import { getThemePoems } from '../api'
import { PageLoader, Empty, ErrorBox } from '../components/UI'
import { ArrowRight, BookOpen, ChevronRight, ChevronLeft, Sparkles } from 'lucide-react'
...
export default function ThemePage() {
  ...
  // Get theme details from local data
  const themeDetails = (themesData?.data || []).find(
    (t: any) => t.slug === slug
  )
  const themeName = themeDetails?.name || slug || 'الموضوع'
  const themePoemsCount = themeDetails?.poemsCount || 0
  const themePoetsCount = themeDetails?.poetsCount || 0

  // Load poems for this theme
  const { data: poemsData, isLoading: loadingPoems, error: poemsErr } = useQuery({
    queryKey: ['theme-poems', slug, page],
    queryFn:  () => getThemePoems(slug!, page),
    enabled:  !!slug,
  ...
  return (
    <div className="max-w-5xl mx-auto px-3 sm:px-6 py-8 sm:py-10" dir="rtl">
      ...
      <div className="flex items-center gap-2 text-xs sm:text-sm text-ink-600 mb-6 sm:mb-8 flex-wrap">
        ...
        <span className="text-ink-400 truncate">{themeName}</span>
      </div>
      ...
      <div className="card-parchment p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8">
        <div className="flex items-start gap-3 sm:gap-5">
          <div className="w-14 h-14 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-full flex-shrink-0
            ...
            <Sparkles className="text-gold-400 w-7 h-7 sm:w-8 sm:h-8" />
          </div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2.5 sm:mb-3">
              <span className="badge-gold text-xs sm:text-sm">{themePoemsCount} قصيدة</span>
              <span className="badge text-xs sm:text-sm">{themePoetsCount} شاعر</span>
            </div>
            <p className="text-ink-400 text-xs sm:text-sm leading-relaxed">
              اكتشف جميع القصائد في موضوع {themeName}
            </p>
          </div>
        </div>
      </div>
      ...
      {!loadingPoems && !poemsErr && (
        <>
          {poems.length === 0 ? (
            <Empty message="لا توجد قصائد لهذا الموضوع" />
          ) : (
